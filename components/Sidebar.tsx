...
const Sidebar = () => {
  ...
  const isAdminPage = pathname?.startsWith("/admin") || pathname?.includes("/admin/") || false;
  const isCoursePage = pathname?.startsWith("/course") || pathname?.includes("/course/") || false;

  const teacherRoutes = [
    { name: "Analytics", href: "/teacher/analytics", icon: ChartBar },
    { name: "Courses", href: "/teacher/courses", icon: BookOpen },
    { name: "Blogs", href: "/teacher/blogs", icon: FileArchive },
  ];
